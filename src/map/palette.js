...
export function reliefStops(snowLine = 2400) {
  return [
    ...
    //
    // **밝게 두면 안 된다.** 처음에 60~66% 로 두었더니 GDAL 로 실제 DEM 에
    // 얹은 그림에서 요단 지구대 전체가 헤르몬 설선(70~92%)과 같은 밝은 회백으로
    // 보였다 — 지구대가 빙하처럼 깔렸다. 명도 검사가 두 구간을 따로 보기 때문에
    // 잡히지 않는 종류의 결함이고, 지도를 실제로 그려 봐야 보인다.
    // 40~50% 의 **따뜻한 회자주**로 내려 눈과 겹치지 않게 한다.
    //
    // 어둡게만 해서는 부족했다 — 검산 ⑥ 이 다시 걸었다. 지구대를 **따뜻한
    // 자주** 로, 암석대를 **차가운 회색** 으로 갈라야 비로소 떨어진다.
    // 색상으로도 갈라 놓는 것이고, 마침 실제 땅빛과도 맞는다(염분지는
    // 붉은 기가 돌고, 고산 석회암 그늘은 푸른 기가 돈다).
    [-430, '#a88a84', '사해 바닥'],
    [-250, '#b99e93', '여리고'],
    [-60, '#cfc3a6', ''],
    ...
    // `checkPalette()` 가 확인한다.
    [rockLine(snowLine), '#8e8c90', '암석대'],
    [snowLine - 100, '#b4b4ba', ''],
    // 실제로 적설
    [snowLine, '#dcdde0', '설선'],
    [snowLine + 400, '#f6f7f9', '헤르몬 2814'],
  ];
}
 ...
 *   ⑥ 멀리 떨어진 두 고도가 같은 색으로 보이지 않는가
 *
 * ⑥ 은 GDAL 로 실제 DEM 에 얹어 보고 나서 넣었다. 사해 바닥과 헤르몬 설선이
 * 둘 다 밝은 회백이라 지구대가 빙하처럼 보였는데, ①~⑤ 는 두 구간을 따로 보기
 * 때문에 전부 통과했다. **검산기가 못 보는 것이 있다는 사실 자체를 검산기에
 * 적어 둔다.**
 */
export function checkPalette(snowLine = 2400) {
  ...
  // ── ⑥ 먼 두 고도가 같은 색으로 보이면 안 된다.
  //     1,000 m 넘게 떨어진 짝을 전부 본다. 사해 바닥(-430)과 설선(2,400)처럼
  //     램프의 양 끝이 우연히 닮는 일이 실제로 있었다.
  for (let i = 0; i < stops.length; i++) {
    for (let j = i + 1; j < stops.length; j++) {
      const [mi, ci] = stops[i];
      const [mj, cj] = stops[j];
      if (mj - mi < 1000) continue;
      const A = hexToRgb(ci);
      const B = hexToRgb(cj);
      const d = Math.max(...A.map((v, k) => Math.abs(v - B[k])));
      if (d < 24) {
        problems.push(
          `${mi} m (${ci}) 과 ${mj} m (${cj}) 이 거의 같은 색이다 — 채널 최대차 ${d}`);
      }
    }
  }
  return problems;
